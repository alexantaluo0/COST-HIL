...
def make_policy(
    ...
    # Note: Skip dataset_stats for SAC policy as it doesn't need it
    if ds_meta is not None and hasattr(ds_meta, "stats") and cfg.type != "sac":
    ...
    if cfg.pretrained_path:
        ...
        # For from_pretrained, only pass config and pretrained_name_or_path
        pretrained_kwargs = {
            "config": kwargs["config"],
            "pretrained_name_or_path": kwargs["pretrained_name_or_path"]
        }
        policy = policy_cls.from_pretrained(**pretrained_kwargs)
    else:
        # Make a fresh policy.
